...
# Sits above Runtime + Garden boxes, below USER LAYER swim lane divider
# ═══════════════════════════════════════════════════════════════════════════════

ax.add_patch(FancyBboxPatch((8.4, 7.80), 7.65, 0.52,
    boxstyle="round,pad=0.05", facecolor='#1a0a3a', edgecolor='#7c3aed', lw=1.5, zorder=4))
# Left accent bar
ax.add_patch(plt.Rectangle((8.4, 7.80), 0.07, 0.52, facecolor='#7c3aed', zorder=5))
# Title
ax.text(12.3, 8.18, 'BOOMI API CONTROL PLANE', color='#c4b5fd', fontsize=8,
        fontweight='bold', ha='center', va='center', zorder=6)
# Capability tags
ax.text(12.3, 7.94, 'Federated Discovery  •  Multi-Gateway Governance  •  OWASP Security  •  OpenAPI Conformance',
        color='#a78bfa', fontsize=8.5, ha='center', va='center', zorder=6, style='italic')
...
# Desk/laptop rect
fancy_rect(0.5, 8.94, 2.0, 0.42, '#1e293b', '#334155', lw=1, pad=0.05, zorder=3)
...
# Outer box
fancy_rect(1.5, 4.6, 3.0, 3.0, '#1e293b', '#7c3aed', lw=2.5, pad=0.1, zorder=3)
...
dh_rows = [
    ('Employee 360',            '#c4b5fd', True),
    ('Profile & Identity',      '#94a3b8', False),
    ('Compensation & Benefits', '#94a3b8', False),
    ('Performance & Engagement','#94a3b8', False),
]
...
ax.text(3.0, 4.88, 'Golden Record', color='#a78bfa', fontsize=8,
        ha='center', va='center', zorder=5, style='italic')

# Labels below
ax.text(3.0, 4.3, 'Boomi Data Hub', color='white', fontsize=11,
        fontweight='bold', ha='center', va='center', zorder=5)
ax.text(3.0, 4.02, 'Employee 360  •  Master Data', color='#94a3b8', fontsize=9,
        ha='center', va='center', zorder=5)

# Step badge 5 — Boomi Data Hub
step_badge(1.4, 7.95, '4', '#7c3aed', size=10)

# Bidirectional push/pull: Data Hub ↔ Boomi Runtime
ax.annotate('', xy=(6.6, 6.1), xytext=(4.5, 6.1),
            arrowprops=dict(arrowstyle='<->', color='#a78bfa', lw=2.0), zorder=6)
ax.text(5.55, 6.35, 'Push / Pull', color='#a78bfa', fontsize=8.5,
        ha='center', va='center', zorder=7)

# Routed lookup: Data Hub → below label texts → Agent Garden box bottom
# Segment 1: drop from Data Hub box bottom-left corner (x=1.5, y=4.5) down to y=3.7
ax.annotate('', xy=(1.5, 3.7), xytext=(1.5, 4.5),
            arrowprops=dict(arrowstyle='-', color='#a78bfa', lw=1.5, linestyle='dashed'), zorder=5)
...
ax.annotate('', xy=(11.5, 4.0), xytext=(11.5, 3.7),
            arrowprops=dict(arrowstyle='->', color='#a78bfa', lw=1.5, linestyle='dashed'), zorder=5)
# Label centered on horizontal segment, left of the orange Data Sources arrow at x=11.8
ax.text(5.55, 3.9, 'Employee 360 Lookup', color='#a78bfa', fontsize=8.5,
        ha='center', va='center', zorder=7,
        bbox=dict(boxstyle='round,pad=0.1', facecolor='#0f172a', edgecolor='none', alpha=0.9))
...
ax.text(13.1, 3.05, 'HR SYSTEMS (Data via Agent Tools)', color='#fbbf24',
